...
export async function getOrgRole(userId, orgId, siteRole) {
  ...
  try {
    ...
    // org_members table may not exist yet — fall back to owner check
    const ownerCheck = await sql`SELECT owner_id FROM organizations WHERE id = ${orgId}`;
    if (ownerCheck.rows[0]?.owner_id == userId) return 'admin';
    // Check if user is a respondent for this org
    const respCheck = await sql`SELECT id FROM respondents WHERE org_id = ${orgId} AND user_id = ${userId} LIMIT 1`;
    if (respCheck.rows.length > 0) return 'respondent';
    return null;
  }
}
